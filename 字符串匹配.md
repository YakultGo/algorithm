...
struct Hash {
    ...
    pair<ll, ll> mul(int l1, int r1, int l2, int r2) {
        pair<ll, ll> it1 = get(l1, r1), it2 = get(l2, r2), it3;
        it1.first *= M1[r2 - l2 + 1] %= Mod[1];
        it1.second *= M2[r2 - l2 + 1] %= Mod[2];
        it3.first = (it1.first + it2.first) % Mod[1];
        it3.second = (it1.second + it2.second) % Mod[2];
        return it3;
    }
